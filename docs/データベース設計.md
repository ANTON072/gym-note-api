...
erDiagram
    ...
    exercises {
        bigint id PK
        string name UK "種目名"
        integer exercise_type "運動タイプ(0:strength,1:cardio)"
...
1. **typeの決定**
   - `workout_exercise.exercise.exercise_type` が `strength` の場合 → `StrengthSet`
   - `workout_exercise.exercise.exercise_type` が `cardio` の場合 → `CardioSet`
